...
const collapseActions = (allActions: Array<IAction>) => {
    ...
    while (i < allActions.length) {
        ...
        if (currentAction.name === 'editor-type' || currentAction.name === 'terminal-type') {
            // Accumulate typed characters
            let accumulatedValue = currentAction.value;
            i++;
            while (
                i < allActions.length &&
                (allActions[i]?.name === 'editor-type' || allActions[i]?.name === 'terminal-type')
            ) {
                if (allActions[i]?.name === 'editor-space' || allActions[i]?.name === 'terminal-space') {
                    accumulatedValue += ' ';
                } else {
